  ...
  <div class="absolute inset-0 overflow-y-hidden bg-black bg-opacity-80">
    <div class="flex items-start justify-center min-h-screen mt-24 text-center">
      ...
      >
        ...
        <h3>Твоят резултат е:</h3>
        <div class="text-6xl font-bold my-4">{{ score }}</div>
        <img
          class="mx-auto"
          :src="imageSource"
          width="480"
          ...
          alt=""
        />
        <div v-if="players.length > 1" class="text-2xl font-bold my-2">
          Играчи
        </div>
        <div
        ...
        >
          ...
          >
            <div class="text-lg">
              ...
              {{ !player.isGameOver ? "(still playing)" : "" }}
            </div>
          </div>
        </div>
        <div class="flex justify-center py-4 text-white">
          ...
          <button
            @click="$emit('share')"
            class="border dark:border-gray-700 border-[#1f1e1e] text-slate-800 dark:text-slate-300 p-4 rounded-lg text-center mr-2"
          >
            Сподели
          </button>
          <button
            @click="$emit('newGame')"
            class="dark:bg-[#1f1e1e] dark:text-slate-300 text-slate-800 bg-gray-300 p-4 rounded-lg text-center"
          >
            Опитай пак
          </button>
        </div>
